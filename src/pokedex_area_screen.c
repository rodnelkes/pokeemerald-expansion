...
static bool8 MonListHasSpecies(const struct WildEncounterTypes *, u16, enum WildPokemonArea);
...
// As a result it's reading out of bounds of the fishing encounters tables.
    ...
    if (MonListHasSpecies(info, species, WILD_AREA_ROCKS))
        return TRUE;
    if (MonListHasSpecies(info, species, WILD_AREA_HIDDEN))
        return TRUE;
    return FALSE;
}

static bool8 MonListHasSpecies(const struct WildEncounterTypes *type, u16 species, enum WildPokemonArea area)
{
    u16 i, size;
    ...
    switch(area){
        case WILD_AREA_WATER:
            info = type->waterMonsInfo;
            size = WATER_WILD_COUNT;
            break;
        case WILD_AREA_ROCKS:
            info = type->rockSmashMonsInfo;
            size = ROCK_WILD_COUNT;
            break;
        case WILD_AREA_FISHING:
            info = type->fishingMonsInfo;
            size = FISH_WILD_COUNT;
            break;
        case WILD_AREA_HIDDEN:
            info = type->hiddenMonsInfo;
            size = HIDDEN_WILD_COUNT;
            break;
        case WILD_AREA_LAND:
            info = type->landMonsInfo;
